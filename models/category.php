...
class Category
{
    private $fields = ['category_id', 'name'];
    ...
    {
        $q = "select {$this->field_str} from {$this->table_name} where `category_id` = ?";
        $stmt = $this->db->prepare($q);
        $stmt->execute([$id]);
    ...
    {
        $q = "update `{$this->table_name}` set name=? where `category_id` = ?";
        $stmt = $this->db->prepare($q);
    ...
    {
        $q = "delete from `{$this->table_name}` where `category_id` = ?";
        $stmt = $this->db->prepare($q);
